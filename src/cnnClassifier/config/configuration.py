...
# for DataIngestionConfig return type , import it from config-entity.py file
...
from cnnClassifier.entity.config_entity import PrepareCallbacksConfig_resnet
...
class ResnetConfigurationManager:
    def __init__(
        self, 
    ...
    def get_prepare_base_model_config(self) -> Prepare_resnet_BaseModelConfig:
        config = self.config.prepare_base_model
        
        create_directories([config.root_dir])

        prepare_base_model_config = Prepare_resnet_BaseModelConfig(
            root_dir=Path(config.root_dir),
            base_model_path=Path(config.base_model_path),
            updated_base_model_path=Path(config.updated_base_model_path),
            params_image_size=self.params.IMAGE_SIZE,
